...
function fazerLogout() {
  ...
  function enviarMensagem() {
    ...
    var storedMessages2 = localStorage.getItem("mensagens2");
  
    if (storedMessages2 !== null) {
      mensagens2 = JSON.parse(storedMessages2);
    }
  
    // Adicionar a nova mensagem à lista
    mensagens2.push(novaMensagem2);
  
    // Armazenar as mensagens atualizadas no arquivo JSON
    localStorage.setItem("mensagens2", JSON.stringify(mensagens2));
  ...
  function exibirMensagens() {
    ...
    if (storedMessages2 !== null) {
      var mensagens2 = JSON.parse(storedMessages2);
  
      // Construir o HTML para exibir as mensagens
      var html = "";
      for (var i = 0; i < mensagens2.length; i++) {
        var mensagem = mensagens2[i];
